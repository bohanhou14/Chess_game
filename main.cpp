...
int main(int argc, char* argv[]) {
	int errorrstate = 0; //record the error state of main, 0 means all good
	Chess::Game game;
	...
	while(!game_over) {
		...
		if (choice.length() != 1) {
			...
			case 'L': case 'l': {
				...
				std::ifstream ifs;
				ifs.open( argument );
				//! this try catch is added by me, may have bugs(I tested it though)——Richard
				try{
					ifs >> game;
				} catch(const Chess::Exception& ex) {
					std::cerr << "Cannot load the game!";
				...
				if(!game.is_valid_game()) {
					std::cerr << "Invalid game loaded!" << std::endl;
					errorrstate = -1;
					game_over = true;
				}
				break;
